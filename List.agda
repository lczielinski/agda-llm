...
notHeadThenInRest a b (inBList , aNotb) =
  let frBList = car inBList
      bkBList = car (cdr inBList)
      eq = cdr (cdr inBList)
      frBListNotNil = λ frBListNil → aNotb (sym $ stupid (trans eq $ trans (cong (λ x → concat x (a :: bkBList)) frBListNil) (concatNil (a :: bkBList))))
      tailConcatEq = tailConcat=ConcatTail frBList (a :: bkBList) frBListNotNil
  in tail frBList , (bkBList , trans (cong tail eq) tailConcatEq)
